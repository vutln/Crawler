...
export class CreateCrawlJobDto {
  ...
  @ApiPropertyOptional({
    ...
    nullable: true,
    description:
      'Hard cap on items collected. null = bounded only by maxPages.',
  })
  @Type(() => Number)
  @IsInt()
  @Min(1)
  ...
  @ApiPropertyOptional({
    description:
      'Standard 5/6-field cron. Omit for manual-only. Example: "0 0 6 * * *" = daily 06:00',
    example: '0 0 6 * * *',
  })
  @IsString()
  @IsOptional()
...
export class CrawlRunDto {
  ...
  @ApiProperty({
    ...
    type: String,
    description:
      'Failure reason, or the anti-bot evidence when status is BLOCKED',
  })
  error!: string | null;
  @ApiProperty({ format: 'date-time' }) createdAt!: string;
  @ApiProperty({
