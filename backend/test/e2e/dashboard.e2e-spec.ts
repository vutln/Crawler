...
describe('Dashboard E2E', () => {
   ...
   */
  itIfUp(
    'keyword paste preview recognises an existing term despite case and spacing',
    async () => {
      try {
        const page = new KeywordsPage(driver);
        await page.open();
        const summary = await page.previewPaste(
          'Mechanical   KEYBOARD\nbrand new term',
        );
  ...
  itIfUp(
    'history page lists runs and takes its filters from the URL',
    async () => {
      try {
