...
const CAPTCHA_MARKERS = /Pardon Our Interruption|splashui\/captcha|Just a moment|are you a robot/i;
...
describe('CANARY: eBay live DOM still matches our selectors', () => {
  ...
  beforeAll(async () => {
    ...
    await driver.get('https://www.ebay.com/sch/i.html?_nkw=vintage+film+camera&_ipg=60');
  ...
  it('finds search result containers', async () => {
    if (blocked) return;
    const items = await driver.findElements(By.css(SELECTORS_UNDER_TEST.resultItem));
  ...
  it('finds a title inside a result', async () => {
    if (blocked) return;
    const items = await driver.findElements(By.css(SELECTORS_UNDER_TEST.resultItem));
    const titles = await items[0].findElements(By.css(SELECTORS_UNDER_TEST.title));
  ...
  it('finds a price inside a result', async () => {
    ...
    for (const item of items.slice(0, 5)) {
      found += (await item.findElements(By.css(SELECTORS_UNDER_TEST.price))).length;
    }
  ...
  it('finds item links matching our id-extraction pattern', async () => {
    ...
    const hrefs = await Promise.all(links.slice(0, 5).map((l) => l.getAttribute('href')));
    ...
    expect(hrefs.some((h) => /\/itm\/(?:[^/]+\/)?(\d{9,15})/.test(h ?? ''))).toBe(true);
  });
});
